...
else
{
				...
				else
				{
					...
					else
					{
						...
						if(resObj.is_admin==1)
						{
							...
							window.open("http://104.196.1.169/admin.html","_self");
						}
						else{
	...
	xmlhttp.open("GET","http://104.196.1.169/login?"+queryObjectToString({uname:uname.value.toLowerCase(),pword:hashValue}));
	xmlhttp.send();
...
document.getElementById("cbutton").addEventListener("click",function() { //MOVING TO SIGN UP PAGE IF BUTTON IS CLICKED
							window.open("http://104.196.1.169/signup.html","_self");
									});
